...
foreach ($name in $required) {
    if (-not (Test-Path -LiteralPath (Join-Path $backup $name) -PathType Leaf)) { throw "Missing backup file: $name" }
}
...
if ($LASTEXITCODE -ne 0) { throw "Database restore failed; application services remain stopped." }
