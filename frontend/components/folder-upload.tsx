...
export function FolderUpload({ csrfToken }: Props) {
  ...
  const [progress, setProgress] = useState<UploadProgress>({ total: 0, uploaded: 0, failed: 0 });
  ...
  function selectFiles(selected: FileList | null) {
    ...
    setError(valid.length ? null : "Select a folder containing JPEG, PNG, or WEBP images.");
    setProcessingStatus(null);
  }

  async function transfer(targetBatchId: string, targetFiles: File[]): Promise<File[]> {
    const failures: File[] = [];
    let cursor = 0;
    ...
    await Promise.all(Array.from({ length: Math.min(5, targetFiles.length) }, worker));
