...
class MeterReader:
    def read(self, image: np.ndarray, lines: list[TextLine] | None = None) -> tuple[str | None, float]:
        ...
        for line in lines if lines is not None else read_lines(image):
            ...
            for group in groups:
                box_height = max(point[1] for point in line.box) - min(point[1] for point in line.box)
                score = min(0.99, line.confidence * 0.86 + min(box_height / max(image.shape[0] * 0.025, 1), 1) * 0.14)
                if score > best[1]:
                    best = (group, round(score, 4))
                    best_line = line
        if best_line and best[0]:
            decimal, decimal_confidence = self._read_decimal_wheel(image, best_line, len(best[0]))
            if decimal:
                return f"{best[0]}.{decimal}", round(min(best[1], decimal_confidence), 4)
            return best
        ...
        for variant in variants:
            for psm in (6, 7, 11, 13):
                ...
                groups = re.findall(r"\d{4,7}(?:[.,]\d)?", candidate.text.replace(" ", ""))
                for group in groups:
                    value = group.replace(",", ".")
                    digits = len(value.replace(".", ""))
                    if not 4 <= digits <= 7:
                        continue
                    score = min(0.94, 0.45 + candidate.confidence * 0.45 + min(digits, 6) * 0.015)
                    if score > best[1]:
                        best = (value, round(score, 4))
        return best

    @staticmethod
    def _read_decimal_wheel(image: np.ndarray, line: TextLine, digit_count: int) -> tuple[str | None, float]:
